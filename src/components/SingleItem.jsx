...
const SingleItem = ({ book }) => {
  ...
  const handleBuyNow = () => {
    console.log("Navigating to payment with book:", book); // For debugging
    if (book && book._id) {
      navigate('/payment', { state: { book: book } });
    } else {
      console.error("Book data is invalid or missing _id, cannot proceed to payment.");
  ...
  return (
    <div className="max-w-6xl w-full px-6">
      <p>
        ...
        <span className="text-indigo-500"> {book.name}</span>
      </p>
      ...
      <div className="flex flex-col md:flex-row gap-16 mt-4">
        <div className="flex gap-3">
          ...
          <div className="border border-gray-500/30 max-w-100 rounded overflow-hidden">
            <img src={book.image} alt="Selected product" />
          </div>
        </div>
        ...
        <div className="text-sm w-full md:w-1/2">
          ...
          <div className="flex items-center gap-0.5 mt-1">
            ...
            <p className="text-base ml-2">({book.rating})</p>
          </div>

          <div className="mt-6">
            <p className="text-gray-500/70 line-through">MRP: ${book.price}</p>
            <p className="text-2xl font-medium">MRP: {book.price}/-</p>
...
SingleItem.propTypes = {
  book: PropTypes.shape({
    _id: PropTypes.string, // Ensure _id is part of prop validation if used
    id: PropTypes.string,
    image: PropTypes.string.isRequired,
    name: PropTypes.string.isRequired,
    rating: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
    price: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
    // Add other book properties if they are used or passed
  }).isRequired,
};
